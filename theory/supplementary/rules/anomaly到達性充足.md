...
各パラダイム P_i（シフト元となるもの）について、固有想起記述素からの同化連鎖によって anomaly を threshold 以上生成できることを保証する規則。

関係構造の構成 Rule 3 の定量的強化として位置づけられる。Rule 3 は「固有想起→共有想起の関係が存在すること」を要求するが、本規則は「anomaly 生成に十分な量であること」を要求する。

## 定義

- **E(P_i)**: P_i の固有想起記述素 = Conceivable(P_i) - ∪_{j≠i} Conceivable(P_j)
...
1. **固有想起記述素の不足**: |E(P_i)| が少なく到達の起点がない → 固有想起記述素の追加
2. **R(P) 接続性の不足**: 固有想起→共有想起の関係がなく到達できない → R(P) 関係の追加
...
- 関係構造の構成 Rule 3 を包含する（anomaly 到達性が充足されれば、固有想起→共有想起の関係の存在は自動的に満たされる）
