...
各パラダイム P（T を除く）の**固有アノマリー記述素**が Q(P) のいずれかの質問の effect でカバーされること。

**定義**:
- Anomaly(P) = {d ∈ 判定域(P) | P_pred(P, d) ≠ T(d)}
- SharedAnomaly(P) = ∪_{P' : depth(P') < depth(P)} Anomaly(P') — P より浅いパラダイム群のアノマリー和集合
- UniqueAnomaly(P) = Anomaly(P) \ SharedAnomaly(P)

UniqueAnomaly(P) の全要素について、d を effect に含む質問 q ∈ Q(P) が存在する。

**根拠**: 共有アノマリー（上位パラダイムも同じ記述素を誤予測している）は、上位パラダイムのフェーズで既に O に正しい観測値が入るため、Q(P) 自身でカバーする必要がない。
