...
- UniqueAnomaly(P) = Anomaly(P) \ ∪_{P' ≠ P} Anomaly(P') — P のみが誤予測する記述素

**局所条件**: 各 P（T を除く）について、UniqueAnomaly(P) の全要素が Q(P) の effect でカバーされること。固有アノマリーは他のパラダイムでは発掘されないため、Q(P) 自身でカバーする必要がある。

**大域条件**: 全パラダイムの全アノマリー記述素 ∪_P Anomaly(P) の各 d について、d を effect に含む質問 q ∈ Q(P') がいずれかの P' に存在すること。共有アノマリーは Q(P) 自身でのカバーを要求しないが、いずれかの Q(P') でカバーされている必要がある。
...
ゲーム進行シミュレーションで、初期オープンから質問の開放が順序的に成立し、各パラダイムの固有アノマリーが発掘されること。共有アノマリー（上位パラダイムと共通のアノマリー記述素）は先行フェーズで発掘済みとみなす。
