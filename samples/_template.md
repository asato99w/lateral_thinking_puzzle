# サンプル生成テンプレート

水平思考パズルに対して理論フレームワークを適用し、POC 取り込み可能なデータを生成するまでの流れ。

## ファイル構成

```
samples/NNN_タイトル/
  01_入力.md            ← Ps, Pt の定義
  02_記述素展開.md      ← 記述素空間の生成
  03_パラダイム発見.md   ← パラダイム群の発見
  04_内部構造分析.md     ← 各パラダイムの D⁺/D⁻/R(P)
  05_質問生成.md        ← 質問群の生成
  06_形式化.md          ← POC 取り込み用の構造化データ
```

## 流れ

### 01_入力

quizzes/ の問題文を元に Ps（問題文の命題分解）と Pt（真相の命題分解）を定義する。

### 02_記述素展開

展開アルゴリズム（E1 等）を Ps・Pt に適用し、記述素空間（Fs, Ft, D, 緊張関係）を生成する。

### 03_パラダイム発見

発見アルゴリズム（A 等）を記述素空間に適用し、メインパス（P1→...→PN=T）とサブパスを発見する。

### 04_内部構造分析

内部構造構成アルゴリズム（A 等）を各パラダイムに適用する。

- D⁺(P): P が真と予測する記述素
- D⁻(P): P が偽と予測する記述素
- R(P): 同化関係（ソース→ターゲット、重み）

アノマリーの単調減少条件（anomalies(O*, P1) > ... > anomalies(O*, PN) = 0）を検証する。

### 05_質問生成

質問生成アルゴリズム（Q1 等）を記述素空間とパラダイム群に適用し、質問群を生成する。各質問に役割（駆動・蓄積・探索・分岐・偶然の開き・クリア）を付与する。

### 06_形式化

02〜05 の分析結果を統合し、POC の JSON データに変換可能な形式に整理する。

- 追加記述素の定義（質問生成で不足した分）
- パラダイムの形式定義（04 の D⁺/D⁻/R(P) を JSON 形式に）
- 質問の ans 定義（ans_yes / ans_no / ans_irrelevant の効果）
- クリア質問の指定

## 運用上の注意

- 各ステップで使用するアルゴリズムをファイル内に明記する
- 後段の検証で不足が見つかった場合、前段に戻って改善する
- 05_形式化の内容を JSON に変換し app/poc/data/ に配置する
