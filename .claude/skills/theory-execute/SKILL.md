...
- **Step 1-4（Explained 包含連鎖の検証）**: 基準パス上で Explained(P_i) ⊂ Explained(P_{i+1}) を検証。不成立なら Step 1-3 に戻り P_pred を修正
...
- **Step 3-2（認知的前提の設定）**: Phase 2 の逆算結果から prerequisites を導出。出力: 各質問の prerequisites（記述素 ID のリスト。値は含めない）
- **Step 3-3（役割付与・大区分配置）**: 役割と大区分を設定。出力: 役割・大区分付き質問群
- **Step 3-4（所属パラダイムの割り当て）**: 各質問の所属パラダイム（Question.paradigms）を設定。出力: Q(P) の決定
- **Step 3-5（フェーズ到達の検証）**: フェーズ到達保証を検証。出力: 検証結果

#### Phase 3 完了チェック

- [ ] 質問群が Phase 2 の層構造と整合している
- [ ] 各質問に prerequisites が設定されている（形式: 記述素 ID のみ、値なし）
- [ ] 各質問に所属パラダイム（Question.paradigms）が設定されている（空は不可）
- [ ] 各質問に役割と大区分が設定されている
- [ ] フェーズ到達保証が検証されている
