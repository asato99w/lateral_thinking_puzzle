...
本プロジェクトには **v1** と **v2** の2つの独立した体系が存在する。両者は完全に独立しており、相互参照してはならない。

| | v1 | v2 |
|---|---|---|
| サンプル | `samples/{lang}/` | なし（サンプル工程なし） |
...
- **サンプル**: `samples/{lang}/` 内の非構造化テキスト。アルゴリズムをパズルに適用する過程で順次生成される。これを元にアプリデータ（JSON）が作成される
...
- v1 の `samples/` を参照してはならない。v2 は独自の設計・データ構造を持つ

### 共通
- **ノート**: `notes/` 内の分析・検討メモ。`YYYY-MM-DD_テーマ.md` の命名規則

## オープンクローズド原則（v1 Phase A に適用）

v1 体系において、アルゴリズムをパズルに適用してサンプルを生成する過程に適用される原則。
...
- 既存の `samples/` 内の他のサンプル（他言語を含む）や `app/poc/data/` 内の JSON データを参考にしてはならない
...
- 変更履歴は `samples/{lang}/{puzzle}/{iteration}/history/` に記録（追記専用）
- 理論・エンジン・eval スクリプトは変更しない

## v2 の作業規則

- v2 データ（`app/poc_v2/samples/`）の変更は eval（`app/poc_v2/eval/`）で検証する
- v1 の `samples/` や `app/poc/` を参照・流用しない
- 変更履歴は `app/poc_v2/samples/{puzzle}/{iteration}/history/` に記録
