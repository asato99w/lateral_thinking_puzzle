...
data_src.json の場合（_ フィールド検出で自動判別）:
  既存チェック + check_chain_consistency + check_recall_fc
data.json の場合:
  既存チェックのみ
"""
...
from check_recall_fc import run as run_recall_fc
...
def main():
    ...
    if is_src and ok_integrity:
        ...
        ok_recall, _ = run_recall_fc(path)
        results.append(("check_recall_fc", ok_recall))
        print()
